...
- **#26383**: `codex exec` не диспатчит project-scope хуки (`<repo>/.codex/hooks.json`) —
  ставить в user-scope `~/.codex/hooks.json`. User-scope безопасен нашим контрактом
  настройки проекта: в каталоге без ключей и без своего `.env.local` хук молча выходит.
  **Важно (с 0.5.0):** ключи, экспортированные в профиле шелла, делают «настроенным»
  любой каталог и эту защиту снимают — тогда user-scope хук Codex начнёт писать
  память во всех проектах подряд. Ключи давать процессу (обёртка секрет-менеджера
  или блок `[mcp_servers.sentgraph.env]` — но он достаётся только MCP-серверу,
  не хукам), а `SENTGRAPH_PROJECT_ID` держать в `.env.local` каждого проекта.
  Глобально экспортированный `SENTGRAPH_PROJECT_ID` перебьёт файловый и сольёт
  все репозитории в один граф.
